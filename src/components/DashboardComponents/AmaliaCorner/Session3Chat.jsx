...
const Session3Chat = ({ isSidebarCollapsed = true, onNextSession, userInitials }) => {
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      ...
      if (currentDomain === "goal") {
        sendResponse = await pathwayService.sendGoalMessageSession3(text, "CORE");
      } else if (currentDomain === "res") {
        sendResponse = await pathwayService.sendResilienceMessageSession3(text, "CORE");
      } else if (currentDomain === "eng") {
        sendResponse = await pathwayService.sendEngagementMessageSession3(text, "CORE");
      } else if (currentDomain === "self") {
        sendResponse = await pathwayService.sendSelfAwarenessMessageSession3(text, "CORE");
      } else if (currentDomain === "belong") {
        sendResponse = await pathwayService.sendBelongingMessageSession3(text, "CORE");
      } else {
        sendResponse = await pathwayService.sendEmpathyMessageSession3(text, "CORE");
      }
